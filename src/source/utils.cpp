...
vector<nc::NdArray<float>> TRTModule::trtInference(nc::NdArray<float> inputData, nc::NdArray<float> imgz){
    ...
    ortInputs += dynamic_cast<float*>((Ort::Float16_t*)(&session->Run(session->GetInputNames(), vector<Ort::Value<float>>((long unsigned)*nc::toStlVector<float>(ortInputs).data()), session->GetOutputNames())));
