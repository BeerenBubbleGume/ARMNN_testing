...
vector<nc::NdArray<float>> TRTModule::trtInference(nc::NdArray<float> inputData, nc::NdArray<float> imgz){
    ...
    ortInputs += reinterpret_cast<float*>(
        reinterpret_cast<Ort::Float16_t*>(session->Run(session->GetInputNames(), vector<Ort::Value>((long unsigned)*nc::toStlVector<float>(ortInputs).data()), session->GetOutputNames())));
