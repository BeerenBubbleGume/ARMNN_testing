...
struct ABC{
    ...
    static nc::NdArray<float> letterbox(nc::NdArray<float> image, vector<float> expected_size);
...
protected:
    ...
    vector<nc::NdArray<float>> trtInference(nc::NdArray<float> intpuData, vector<float> imgz);
